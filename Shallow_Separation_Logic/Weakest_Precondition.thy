...
urust_expr [abbrev] wp_pause_expr1
  \<open> \<y>\<i>\<e>\<l>\<d> \<close>

lemma wp_pause:
    notes asepconj_simp [simp]
  assumes \<open>\<And>r. ucincl (\<psi> r)\<close>
    shows \<open>\<W>\<P> \<Gamma> wp_pause_expr1 \<psi> \<rho> \<theta> = \<psi> ()\<close>
...
\<comment>\<open>NOTE: This lemma is not used at present, but seems worth keeping.\<close>
urust_expr [abbrev] wp_pauseI_expr1
  \<open> \<y>\<i>\<e>\<l>\<d> \<close>

lemma wp_pauseI:
  assumes \<open>\<And>r. ucincl (\<psi> r)\<close>
      and \<open>\<phi> \<longlongrightarrow> \<psi> ()\<close>
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> wp_pauseI_expr1 \<psi> \<rho> \<theta>\<close>
using assms by (simp add: wp_pause)

urust_expr [abbrev] wp_log_expr1
  (p, l)
...
lemma wp_log:
    notes asepconj_simp [simp]
  assumes \<open>\<And>r. ucincl (\<psi> r)\<close>
    shows \<open>\<W>\<P> \<Gamma> (wp_log_expr1 p l) \<psi> \<rho> \<theta> = \<psi> ()\<close>
using assms by (auto intro!: aentails_yonedaI simp add: sstriple_wp_iff sstriple_log')

urust_expr [abbrev] wp_logI_expr1
  (p, l)
  \<open> \<l>\<o>\<g> \<llangle>p\<rrangle> \<llangle>l\<rrangle> \<close>
...
lemma wp_logI [micro_rust_wp_intros]:
  assumes \<open>\<And>r. ucincl (\<psi> r)\<close>
      and \<open>\<phi> \<longlongrightarrow> \<psi> ()\<close>
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_logI_expr1 p l) \<psi> \<rho> \<theta>\<close>
using assms by (simp add: wp_log)

urust_expr [abbrev] wp_fatalI_expr1
  (msg)
...
lemma wp_fatalI [micro_rust_wp_intros]:
  ...
  assumes \<open>is_aborting_striple_context \<Gamma>\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_fatalI_expr1 msg) \<psi> \<rho> \<theta>\<close>
...
urust_expr [abbrev] wp_panic_expr1
  (m)
  \<open> panic!(m) \<close>

corollary wp_panic [micro_rust_wp_simps]:
  shows \<open>\<W>\<P> \<Gamma> (wp_panic_expr1 m) \<psi> \<rho> \<theta> = \<theta> (Panic m) \<star> UNIV\<close>
  by (intro wp_abort)
...
urust_expr [abbrev] wp_panicI_expr1
  (m)
  \<open> panic!(m) \<close>

lemma wp_panicI [micro_rust_wp_intros]:
  assumes \<open>\<phi> \<longlongrightarrow> \<theta> (Panic m) \<star> UNIV\<close>
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_panicI_expr1 m) \<psi> \<rho> \<theta>\<close>
using assms by (subst wp_panic)

urust_expr [abbrev] wp_return_expr1
  (v)
  \<open> return v; \<close>

lemma wp_return [micro_rust_wp_simps]:
    shows \<open>\<W>\<P> \<Gamma> (wp_return_expr1 v) \<psi> \<rho> \<theta> = \<rho> v \<star> \<top>\<close>
by (auto intro!: aentails_yonedaI simp add: sstriple_wp_iff sstriple_return)

urust_expr [abbrev] wp_returnI_expr1
  (v)
  \<open> return v; \<close>

lemma wp_returnI [micro_rust_wp_intros]:
  assumes \<open>\<phi> \<longlongrightarrow> \<rho> v \<star> \<top>\<close>
  shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_returnI_expr1 v) \<psi> \<rho> \<theta>\<close>
using assms by (simp add: wp_return)

urust_expr [abbrev] wp_assert_expr1
  (x)
  \<open> assert!(x) \<close>

lemma wp_assert [micro_rust_wp_simps]:
  shows \<open>\<W>\<P> \<Gamma> (wp_assert_expr1 x) \<psi> \<rho> \<theta> = (\<langle>x\<rangle> \<Zsurj> (UNIV \<star> \<psi> ())) \<sqinter> (\<langle>\<not>x\<rangle> \<Zsurj> (UNIV \<star> \<theta> AssertionFailed))\<close>
...
urust_expr [abbrev] wp_assert_eq_expr1
  (x, y)
  \<open> assert_eq!(x, y) \<close>

lemma wp_assert_eq [micro_rust_wp_simps]:
  shows \<open>\<W>\<P> \<Gamma> (wp_assert_eq_expr1 x y) \<psi> \<rho> \<theta> =
...
urust_expr [abbrev] wp_assertI_expr1
  (x)
  \<open> assert!(x) \<close>
...
lemma wp_assertI[micro_rust_wp_intros]:
  ...
  shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_assertI_expr1 x) \<psi> \<rho> \<theta>\<close>
proof -
...
urust_expr [abbrev] wp_assert_eqI_expr1
  (x, y)
  \<open> assert_eq!(x,y) \<close>
...
lemma wp_assert_eqI[micro_rust_wp_intros]:
  ...
  shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_assert_eqI_expr1 x y) \<psi> \<rho> \<theta>\<close>
...
urust_expr [abbrev] wp_assert_neI_expr1
  (x, y)
...
lemma wp_assert_neI[micro_rust_wp_intros]:
  ...
  shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_assert_neI_expr1 x y) \<psi> \<rho> \<theta>\<close>
  using assms by (clarsimp intro!: wp_assertI[simplified assert_def micro_rust_simps]
     simp add: assert_ne_def assert_ne_val_def micro_rust_simps)

urust_expr [abbrev] wp_bind_core_expr1
  (e, f)
  \<open> let v = \<epsilon>\<open>e\<close>; \<epsilon>\<open>f v\<close> \<close>

lemma wp_bind_core:
  shows \<open>\<W>\<P> \<Gamma> e (\<lambda>r. \<W>\<P> \<Gamma> (f r) \<xi> \<rho> \<theta>) \<rho> \<theta> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_bind_core_expr1 e f) \<xi> \<rho> \<theta>\<close>
...
urust_expr [abbrev] wp_bindI_expr1
  (e, f)
  \<open> let v = \<epsilon>\<open>e\<close>; \<epsilon>\<open>f v\<close> \<close>
...
lemma wp_bindI [micro_rust_wp_intros]:
  ...
  assumes \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> e (\<lambda>r. \<W>\<P> \<Gamma> (f r) \<xi> \<rho> \<theta>) \<rho> \<theta>\<close>
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_bindI_expr1 e f)  \<xi> \<rho> \<theta>\<close>
...
lemma wp_two_armed_conditionalI[micro_rust_wp_case_splits]:
  assumes \<open>x \<Longrightarrow> \<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> t \<psi> \<rho> \<theta>\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_two_armed_conditionalI_expr1 x t f) \<psi> \<rho> \<theta>\<close>
by (simp add: assms wp_two_armed_conditional)

urust_expr [abbrev] wp_one_armed_conditional_expr1
  (x, t)
  \<open> if x { \<epsilon>\<open>t\<close> } \<close>

corollary wp_one_armed_conditional:
  assumes \<open>\<And>s. ucincl (\<psi> s)\<close>
  shows \<open>\<W>\<P> \<Gamma> (wp_one_armed_conditional_expr1 x t) \<psi> \<rho> \<theta> = (if x then \<W>\<P> \<Gamma> t \<psi> \<rho> \<theta> else \<psi> ())\<close>
using assms by (simp add: micro_rust_wp_simps wp_two_armed_conditional)

urust_expr [abbrev] wp_one_armed_conditionalI_expr1
  (x, t)
  \<open> if x { \<epsilon>\<open>t\<close> } \<close>
...
lemma wp_one_armed_conditionalI[micro_rust_wp_case_splits]:
  assumes \<open>x \<Longrightarrow> \<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> t \<psi> \<rho> \<theta>\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_one_armed_conditionalI_expr1 x t) \<psi> \<rho> \<theta>\<close>
  using assms by (simp add: wp_literalI wp_two_armed_conditional)

urust_expr [abbrev] wp_two_armed_conditional_thenI_expr1
  (x, t, f)
  \<open> if x { \<epsilon>\<open>t\<close> } else { \<epsilon>\<open>f\<close> } \<close>
...
lemma wp_two_armed_conditional_thenI:
  assumes \<open>x\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_two_armed_conditional_thenI_expr1 x t f) \<psi> \<rho> \<theta>\<close>
  by (simp add: assms wp_two_armed_conditional)

urust_expr [abbrev] wp_two_armed_conditional_elseI_expr1
  (x, t, f)
  \<open> if x { \<epsilon>\<open>t\<close> } else { \<epsilon>\<open>f\<close> } \<close>
...
lemma wp_two_armed_conditional_elseI:
  assumes \<open>\<not>x\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_two_armed_conditional_elseI_expr1 x t f) \<psi> \<rho> \<theta>\<close>
  by (simp add: assms wp_two_armed_conditional)
...
lemma wp_fun [micro_rust_wp_simps]:
  shows \<open>\<W>\<P> \<Gamma> (f\<langle>\<rangle>) \<phi> \<rho> \<theta> = \<W>\<P> \<Gamma> (call f) \<phi> \<rho> \<theta>\<close>
    and \<open>\<W>\<P> \<Gamma> (wp_fun_expr1 f1 a0) \<phi> \<rho> \<theta> = \<W>\<P> \<Gamma> (wp_fun_expr2 f1 a0) \<phi> \<rho> \<theta>\<close>
    and \<open>\<W>\<P> \<Gamma> (wp_fun_expr3 f2 a0 a1) \<phi> \<rho> \<theta> = \<W>\<P> \<Gamma> (wp_fun_expr4 f2 a0 a1) \<phi> \<rho> \<theta>\<close>
    and \<open>\<W>\<P> \<Gamma> (wp_fun_expr5 f3 a0 a1 a2) \<phi> \<rho> \<theta> = \<W>\<P> \<Gamma> (wp_fun_expr6 f3 a0 a1 a2) \<phi> \<rho> \<theta>\<close>
    and \<open>\<W>\<P> \<Gamma> (wp_fun_expr7 f4 a0 a1 a2 a3) \<phi> \<rho> \<theta> = \<W>\<P> \<Gamma> (wp_fun_expr8 f4 a0 a1 a2 a3) \<phi> \<rho> \<theta>\<close>
    and \<open>\<W>\<P> \<Gamma> (wp_fun_expr9 f5 a0 a1 a2 a3 a4) \<phi> \<rho> \<theta> = \<W>\<P> \<Gamma> (wp_fun_expr10 f5 a0 a1 a2 a3 a4) \<phi> \<rho> \<theta>\<close>
    and \<open>\<W>\<P> \<Gamma> (wp_fun_expr11 f6 a0 a1 a2 a3 a4 a5) \<phi> \<rho> \<theta> = \<W>\<P> \<Gamma> (wp_fun_expr12 f6 a0 a1 a2 a3 a4 a5) \<phi> \<rho> \<theta>\<close>
    and \<open>\<W>\<P> \<Gamma> (wp_fun_expr13 f7 a0 a1 a2 a3 a4 a5 a6) \<phi> \<rho> \<theta> = \<W>\<P> \<Gamma> (wp_fun_expr14 f7 a0 a1 a2 a3 a4 a5 a6) \<phi> \<rho> \<theta>\<close>
    and \<open>\<W>\<P> \<Gamma> (wp_fun_expr15 f8 a0 a1 a2 a3 a4 a5 a6 a7) \<phi> \<rho> \<theta> = \<W>\<P> \<Gamma> (wp_fun_expr16 f8 a0 a1 a2 a3 a4 a5 a6 a7) \<phi> \<rho> \<theta>\<close>
    and \<open>\<W>\<P> \<Gamma> (wp_fun_expr17 f9 a0 a1 a2 a3 a4 a5 a6 a7 a8) \<phi> \<rho> \<theta> =
           \<W>\<P> \<Gamma> (wp_fun_expr18 f9 a0 a1 a2 a3 a4 a5 a6 a7 a8) \<phi> \<rho> \<theta>\<close>
    and \<open>\<W>\<P> \<Gamma> (wp_fun_expr19 f10 a0 a1 a2 a3 a4 a5 a6 a7 a8 a9) \<phi> \<rho> \<theta> =
           \<W>\<P> \<Gamma> (wp_fun_expr20 f10 a0 a1 a2 a3 a4 a5 a6 a7 a8 a9) \<phi> \<rho> \<theta>\<close>
...
urust_expr [abbrev] wp_funliteral_expr1
  (f1, v0)
  \<open> \<llangle>f1\<rrangle>\<^sub>1 (v0) \<close>
urust_expr [abbrev] wp_funliteral_expr2
  (f2, v0, v1)
  \<open> \<llangle>f2\<rrangle>\<^sub>2 (v0, v1) \<close>
urust_expr [abbrev] wp_funliteral_expr3
  (f3, v0, v1, v2)
  \<open> \<llangle>f3\<rrangle>\<^sub>3 (v0, v1, v2) \<close>
urust_expr [abbrev] wp_funliteral_expr4
  (f4, v0, v1, v2, v3)
  \<open> \<llangle>f4\<rrangle>\<^sub>4 (v0, v1, v2, v3) \<close>
urust_expr [abbrev] wp_funliteral_expr5
  (f5, v0, v1, v2, v3, v4)
  \<open> \<llangle>f5\<rrangle>\<^sub>5 (v0, v1, v2, v3, v4) \<close>
urust_expr [abbrev] wp_funliteral_expr6
  (f6, v0, v1, v2, v3, v4, v5)
  \<open> \<llangle>f6\<rrangle>\<^sub>6 (v0, v1, v2, v3, v4, v5) \<close>
urust_expr [abbrev] wp_funliteral_expr7
  (f7, v0, v1, v2, v3, v4, v5, v6)
  \<open> \<llangle>f7\<rrangle>\<^sub>7 (v0, v1, v2, v3, v4, v5, v6) \<close>
urust_expr [abbrev] wp_funliteral_expr8
  (f8, v0, v1, v2, v3, v4, v5, v6, v7)
  \<open> \<llangle>f8\<rrangle>\<^sub>8 (v0, v1, v2, v3, v4, v5, v6, v7) \<close>

lemma wp_funliteral:
  assumes \<open>\<And>r. ucincl (\<psi> r)\<close>
    shows \<open>\<W>\<P> \<Gamma> (wp_funliteral_expr1 f1 v0) \<psi> \<rho> \<theta> = \<psi> (f1 v0)\<close>
      and \<open>\<W>\<P> \<Gamma> (wp_funliteral_expr2 f2 v0 v1) \<psi> \<rho> \<theta> = \<psi> (f2 v0 v1)\<close>
      and \<open>\<W>\<P> \<Gamma> (wp_funliteral_expr3 f3 v0 v1 v2) \<psi> \<rho> \<theta> = \<psi> (f3 v0 v1 v2)\<close>
      and \<open>\<W>\<P> \<Gamma> (wp_funliteral_expr4 f4 v0 v1 v2 v3) \<psi> \<rho> \<theta> = \<psi> (f4 v0 v1 v2 v3)\<close>
      and \<open>\<W>\<P> \<Gamma> (wp_funliteral_expr5 f5 v0 v1 v2 v3 v4) \<psi> \<rho> \<theta> = \<psi> (f5 v0 v1 v2 v3 v4)\<close>
      and \<open>\<W>\<P> \<Gamma> (wp_funliteral_expr6 f6 v0 v1 v2 v3 v4 v5) \<psi> \<rho> \<theta> = \<psi> (f6 v0 v1 v2 v3 v4 v5)\<close>
      and \<open>\<W>\<P> \<Gamma> (wp_funliteral_expr7 f7 v0 v1 v2 v3 v4 v5 v6) \<psi> \<rho> \<theta> = \<psi> (f7 v0 v1 v2 v3 v4 v5 v6)\<close>
      and \<open>\<W>\<P> \<Gamma> (wp_funliteral_expr8 f8 v0 v1 v2 v3 v4 v5 v6 v7) \<psi> \<rho> \<theta> = \<psi> (f8 v0 v1 v2 v3 v4 v5 v6 v7)\<close>
...
urust_expr [abbrev] wp_op_eq_expr1
  (e, f)
...
lemma wp_op_eq [micro_rust_wp_simps]:
  assumes \<open>\<And>r. ucincl (\<psi> r)\<close>
    ...
    shows \<open>\<W>\<P> \<Gamma> (wp_op_eq_expr1 e f) \<psi> \<rho> \<theta> = \<psi> (e = f)\<close>
using assms by (clarsimp simp add: wp_funliteral)

urust_expr [abbrev] wp_op_eqs_expr1
  (e, f)
  \<open> e == f \<close>
...
lemma wp_op_eqs [micro_rust_wp_simps]:
  assumes \<open>\<And>r. ucincl (\<psi> r)\<close>
      and \<open>\<And>r. ucincl (\<rho> r)\<close>
    shows \<open>\<W>\<P> \<Gamma> (wp_op_eqs_expr1 e f) \<psi> \<rho> \<theta> = \<psi> (e = f)\<close>
using assms by (clarsimp simp add: urust_eq_def micro_rust_wp_simps micro_rust_simps)

urust_expr [abbrev] wp_word_add_no_wrap_expr1
  (x, y)
  \<open> x + y \<close>
lemma wp_word_add_no_wrap [micro_rust_wp_intros]:
  ...
  assumes \<open>ucincl (\<psi> (x+y))\<close>
    shows \<open>\<langle>unat x + unat y < 2^LENGTH('l)\<rangle> \<star> \<psi> (x + y) \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_word_add_no_wrap_expr1 x y) \<psi> \<rho> \<theta>\<close> (is \<open>?asm \<longlongrightarrow> ?GOAL\<close>)
proof -
  ...
  moreover from calculation and assms have I: \<open>\<Gamma> ; ?asm \<turnstile> wp_word_add_no_wrap_expr1 x y \<stileturn> (\<lambda>r. \<langle>r = x + y\<rangle> \<star> \<psi> (x+y)) \<bowtie> \<rho> \<bowtie> \<theta>\<close>
    by (intro sstriple_frame_rule_no_return; simp add: sstriple_word_add_no_wrapI)
  moreover from assms have \<open>\<Gamma> ; ?asm \<turnstile> wp_word_add_no_wrap_expr1 x y \<stileturn> \<psi> \<bowtie> \<rho> \<bowtie> \<theta>\<close>
...
urust_expr [abbrev] wp_word_add_no_wrapI_expr1
  (x, y)
  \<open> x + y \<close>
...
lemma wp_word_add_no_wrapI [micro_rust_wp_intros]:
  ...
  assumes \<open>ucincl (\<psi> (x + y))\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_word_add_no_wrapI_expr1 x y) \<psi> \<rho> \<theta>\<close>
using assms by - (rule aentails_trans', rule wp_word_add_no_wrap, auto)

urust_expr [abbrev] wp_word_mul_no_wrap_expr1
  (x, y)
  \<open> x * y \<close>
lemma wp_word_mul_no_wrap [micro_rust_wp_intros]:
    ...
    fixes x y :: \<open>'l::{len} word\<close>
  assumes \<open>ucincl (\<psi> (x*y))\<close>
    shows \<open>\<langle>unat x * unat y < 2^LENGTH('l)\<rangle> \<star> \<psi> (x * y) \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_word_mul_no_wrap_expr1 x y) \<psi> \<rho> \<theta>\<close> (is \<open>?asm \<longlongrightarrow> ?GOAL\<close>)
proof -
  have \<open>ucincl (\<langle>(unat x) * (unat y) < 2^(LENGTH('l))\<rangle>)\<close>
    by (simp add: ucincl_apure)
  moreover from this have \<open>ucincl ?asm\<close>
    by (simp add: assms ucincl_apure ucincl_asepconj)
  moreover from calculation and assms have I: \<open>\<Gamma> ; ?asm \<turnstile> wp_word_mul_no_wrap_expr1 x y \<stileturn> (\<lambda>r. \<langle>r = x * y\<rangle> \<star> \<psi> (x*y)) \<bowtie> \<rho> \<bowtie> \<theta>\<close>
    by (intro sstriple_frame_rule_no_return) (auto simp add: sstriple_word_mul_no_wrapI)
  moreover from assms have \<open>\<Gamma> ; ?asm \<turnstile> wp_word_mul_no_wrap_expr1 x y \<stileturn> \<psi> \<bowtie> \<rho> \<bowtie> \<theta>\<close>
    by (intro sstriple_consequence[OF I]; auto)
  ultimately show \<open>?asm \<longlongrightarrow> ?GOAL\<close>
    by (intro wp_is_weakest_precondition)
qed

urust_expr [abbrev] wp_word_mul_no_wrapI_expr1
  (x, y)
  \<open> x * y \<close>
...
lemma wp_word_mul_no_wrapI [micro_rust_wp_intros]:
  ...
  assumes \<open>ucincl (\<psi> (x * y))\<close>
      and \<open>\<phi> \<longlongrightarrow> \<langle>unat x * unat y < 2^LENGTH('l)\<rangle> \<star> \<psi> (x * y)\<close>
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_word_mul_no_wrapI_expr1 x y) \<psi> \<rho> \<theta>\<close>
using assms by - (rule aentails_trans', rule wp_word_mul_no_wrap, auto)

urust_expr [abbrev] wp_word_sub_no_wrap_expr1
  (x, y)
  \<open> x - y \<close>
lemma wp_word_sub_no_wrap:
    ...
    fixes x y :: \<open>'l::{len} word\<close>
  assumes \<open>ucincl (\<psi> (x - y))\<close>
    shows \<open>\<langle>y \<le> x\<rangle> \<star> \<psi> (x - y) \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_word_sub_no_wrap_expr1 x y) \<psi> \<rho> \<theta>\<close> (is \<open>?asm \<longlongrightarrow> ?GOAL\<close>)
proof -
  have \<open>ucincl (\<langle>y \<le> x\<rangle>)\<close>
    by (simp add: ucincl_apure)
  moreover from this have \<open>ucincl ?asm\<close>
    by (simp add: assms ucincl_apure ucincl_asepconj)
  moreover from calculation and assms have I: \<open>\<Gamma> ; ?asm \<turnstile> wp_word_sub_no_wrap_expr1 x y \<stileturn> (\<lambda>r. \<langle>r = x - y\<rangle> \<star> \<psi> (x - y)) \<bowtie> \<rho> \<bowtie> \<theta>\<close>
    by (intro sstriple_frame_rule_no_return) (auto simp add: sstriple_word_sub_no_wrapI)
  moreover from assms have \<open>\<Gamma> ; ?asm \<turnstile> wp_word_sub_no_wrap_expr1 x y \<stileturn> \<psi> \<bowtie> \<rho> \<bowtie> \<theta>\<close>
    by (intro sstriple_consequence[OF I]) auto
  ultimately show \<open>?asm \<longlongrightarrow> ?GOAL\<close>
    by (intro wp_is_weakest_precondition)
qed

urust_expr [abbrev] wp_word_sub_no_wrapI_expr1
  (x, y)
  \<open> x - y \<close>
...
lemma wp_word_sub_no_wrapI [micro_rust_wp_intros]:
  ...
  assumes \<open>ucincl (\<psi> (x - y))\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_word_sub_no_wrapI_expr1 x y) \<psi> \<rho> \<theta>\<close>
using assms by - (rule aentails_trans', rule wp_word_sub_no_wrap, auto)

urust_expr [abbrev] wp_word_udiv_expr1
  (x, y)
  \<open> x / y \<close>
lemma wp_word_udiv:
    notes aentails_intro [intro]
  ...
  shows \<open>\<langle>y \<noteq> 0\<rangle> \<star> \<psi> (x div y) \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_word_udiv_expr1 x y) \<psi> \<rho> \<theta>\<close> (is \<open>?asm \<longlongrightarrow> ?GOAL\<close>)
proof -
  have \<open>ucincl (\<langle>y \<noteq> 0\<rangle>)\<close>
    by (simp add: ucincl_apure)
  moreover from this have \<open>ucincl ?asm\<close>
    by (simp add: assms ucincl_apure ucincl_asepconj)
  moreover from calculation and assms have I: \<open>\<Gamma> ; ?asm \<turnstile> wp_word_udiv_expr1 x y \<stileturn> (\<lambda>r. \<langle>r = x div y\<rangle> \<star> \<psi> (x div y)) \<bowtie> \<rho> \<bowtie> \<theta>\<close>
    by (intro sstriple_frame_rule_no_return) (auto simp add: sstriple_word_udivI)
  moreover from assms have \<open>\<Gamma> ; ?asm \<turnstile> wp_word_udiv_expr1 x y \<stileturn> \<psi> \<bowtie> \<rho> \<bowtie> \<theta>\<close>
    by (intro sstriple_consequence[OF I]) auto
  ultimately show \<open>?asm \<longlongrightarrow> ?GOAL\<close>
    by (intro wp_is_weakest_precondition)
qed

urust_expr [abbrev] wp_word_udivI_expr1
  (x, y)
  \<open> x / y \<close>
...
lemma wp_word_udivI [micro_rust_wp_intros]:
  ...
  assumes \<open>ucincl (\<psi> (x div y))\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_word_udivI_expr1 x y) \<psi> \<rho> \<theta>\<close>
using assms by - (rule aentails_trans', rule wp_word_udiv, auto)

urust_expr [abbrev] wp_word_umod_expr1
  (x, y)
  \<open> x % y \<close>
lemma wp_word_umod:
    notes aentails_intro [intro]
  ...
  assumes \<open>ucincl (\<psi> (x mod y))\<close>
    shows \<open>\<langle>y \<noteq> 0\<rangle> \<star> \<psi> (x mod y) \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_word_umod_expr1 x y) \<psi> \<rho> \<theta>\<close> (is \<open>?asm \<longlongrightarrow> ?GOAL\<close>)
proof -
  have \<open>ucincl (\<langle>y \<noteq> 0\<rangle>)\<close>
    by (simp add: ucincl_apure)
  moreover from this have \<open>ucincl ?asm\<close>
    by (simp add: assms ucincl_apure ucincl_asepconj)
  moreover from calculation and assms have I: \<open>\<Gamma> ; ?asm \<turnstile> wp_word_umod_expr1 x y \<stileturn> (\<lambda>r. \<langle>r = x mod y\<rangle> \<star> \<psi> (x mod y)) \<bowtie> \<rho> \<bowtie> \<theta>\<close>
    by (intro sstriple_frame_rule_no_return) (auto simp add: sstriple_word_umodI)
  moreover from assms have \<open>\<Gamma> ; ?asm \<turnstile> wp_word_umod_expr1 x y \<stileturn> \<psi> \<bowtie> \<rho> \<bowtie> \<theta>\<close>
    by (intro sstriple_consequence[OF I]) auto
  ultimately show \<open>?asm \<longlongrightarrow> ?GOAL\<close>
    by (intro wp_is_weakest_precondition)
qed

urust_expr [abbrev] wp_word_umodI_expr1
  (x, y)
  \<open> x % y \<close>
...
lemma wp_word_umodI [micro_rust_wp_intros]:
  ...
  assumes \<open>ucincl (\<psi> (x mod y))\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_word_umodI_expr1 x y) \<psi> \<rho> \<theta>\<close>
using assms by - (rule aentails_trans', rule wp_word_umod, auto)

urust_expr [abbrev] wp_bitwise_or_expr1
  (x, y)
  \<open> x | y \<close>

lemma wp_bitwise_or:
  assumes \<open>\<And>v. ucincl (\<psi> v)\<close>
    shows \<open>\<psi> (x OR y) \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_bitwise_or_expr1 x y) \<psi> \<rho> \<theta>\<close>
using assms by (intro sstriple_pure_to_wp') (auto intro: sstriple_bitwise_orI)

urust_expr [abbrev] wp_bitwise_and_expr1
  (x, y)
  \<open> x & y \<close>

lemma wp_bitwise_and:
  assumes \<open>\<And>v. ucincl (\<psi> v)\<close>
    shows \<open>\<psi> (x AND y) \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_bitwise_and_expr1 x y) \<psi> \<rho> \<theta>\<close>
using assms by (intro sstriple_pure_to_wp') (auto intro: sstriple_bitwise_andI)

urust_expr [abbrev] wp_bitwise_xor_expr1
  (x, y)
  \<open> x ^ y \<close>

lemma wp_bitwise_xor:
  assumes \<open>\<And>v. ucincl (\<psi> v)\<close>
    shows \<open>\<psi> (x XOR y) \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_bitwise_xor_expr1 x y) \<psi> \<rho> \<theta>\<close>
using assms by (intro sstriple_pure_to_wp') (auto intro: sstriple_bitwise_xorI)

urust_expr [abbrev] wp_bitwise_not_expr1 ::
...
lemma wp_bitwise_not:
  assumes \<open>\<And>v. ucincl (\<psi> v)\<close>
    shows \<open>\<psi> (NOT x) \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_bitwise_not_expr1 x) \<psi> \<rho> \<theta>\<close>
using assms by (intro sstriple_pure_to_wp') (auto intro: sstriple_bitwise_notI)

urust_expr [abbrev] wp_bitwise_orI_expr1
  (x, y)
  \<open> x | y \<close>
...
lemma wp_bitwise_orI [micro_rust_wp_intros]:
  assumes \<open>\<And>v. ucincl (\<psi> v)\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_bitwise_orI_expr1 x y) \<psi> \<rho> \<theta>\<close>
using assms wp_bitwise_or aentails_trans' by blast

urust_expr [abbrev] wp_bitwise_andI_expr1
  (x, y)
  \<open> x & y \<close>
...
lemma wp_bitwise_andI [micro_rust_wp_intros]:
  assumes \<open>\<And>v. ucincl (\<psi> v)\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_bitwise_andI_expr1 x y) \<psi> \<rho> \<theta>\<close>
using assms wp_bitwise_and aentails_trans' by blast

urust_expr [abbrev] wp_bitwise_xorI_expr1
  (x, y)
  \<open> x ^ y \<close>
...
lemma wp_bitwise_xorI [micro_rust_wp_intros]:
  assumes \<open>\<And>v. ucincl (\<psi> v)\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_bitwise_xorI_expr1 x y) \<psi> \<rho> \<theta>\<close>
using assms wp_bitwise_xor aentails_trans' by blast

urust_expr [abbrev] wp_bitwise_notI_expr1 ::
  \<open>'l::len word \<Rightarrow> ('s, 'l word, 'r, 'abort, 'i, 'o) expression\<close>
  (x)
  \<open> !x \<close>
...
lemma wp_bitwise_notI [micro_rust_wp_intros]:
  assumes \<open>\<And>v. ucincl (\<psi> v)\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_bitwise_notI_expr1 x) \<psi> \<rho> \<theta>\<close>
using assms wp_bitwise_not aentails_trans' by blast

urust_expr [abbrev] wp_word_shift_left_expr1
  (x, y)
  \<open> x << y \<close>
lemma wp_word_shift_left:
  ...
  assumes \<open>ucincl (\<psi> (push_bit (unat y) x))\<close>
    shows \<open>\<langle>unat y < LENGTH('l0)\<rangle> \<star> \<psi> (push_bit (unat y) x) \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_word_shift_left_expr1 x y) \<psi> \<rho> \<theta>\<close> (is \<open>?asm \<longlongrightarrow> ?GOAL\<close>)
proof -
  ...
  moreover from calculation and assms have
         I: \<open>\<Gamma> ; ?asm \<turnstile> wp_word_shift_left_expr1 x y \<stileturn> (\<lambda>r. \<langle>r = push_bit (unat y) x\<rangle> \<star> \<psi> (push_bit (unat y) x)) \<bowtie> \<rho> \<bowtie> \<theta>\<close>
    by (intro sstriple_frame_rule_no_return) (auto simp add: sstriple_word_shift_leftI)
  moreover from assms have \<open>\<Gamma> ; ?asm \<turnstile> wp_word_shift_left_expr1 x y \<stileturn> \<psi> \<bowtie> \<rho> \<bowtie> \<theta>\<close>
    by (intro sstriple_consequence[OF I]) auto
  ultimately show \<open>?asm \<longlongrightarrow> ?GOAL\<close>
    by (intro wp_is_weakest_precondition)
qed

urust_expr [abbrev] wp_word_shift_leftI_expr1
  (x, y)
  \<open> x << y \<close>
...
lemma wp_word_shift_leftI [micro_rust_wp_intros]:
  ...
  assumes \<open>ucincl (\<psi> (push_bit (unat y) x))\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_word_shift_leftI_expr1 x y) \<psi> \<rho> \<theta>\<close>
using assms wp_word_shift_left aentails_trans' by blast

urust_expr [abbrev] wp_word_shift_right_expr1
  (x, y)
  \<open> x >> y \<close>
lemma wp_word_shift_right:
  ...
  assumes \<open>ucincl (\<psi> (drop_bit (unat y) x))\<close>
    shows \<open>\<langle>unat y < LENGTH('l0)\<rangle> \<star> \<psi> (drop_bit (unat y) x) \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_word_shift_right_expr1 x y) \<psi> \<rho> \<theta>\<close> (is \<open>?asm \<longlongrightarrow> ?GOAL\<close>)
proof -
  have \<open>ucincl (\<langle>unat y < LENGTH('l0)\<rangle>)\<close>
    by (simp add: ucincl_apure)
  moreover from this have \<open>ucincl ?asm\<close>
    by (simp add: assms ucincl_apure ucincl_asepconj)
  moreover from calculation and assms have
         I: \<open>\<Gamma> ; ?asm \<turnstile> wp_word_shift_right_expr1 x y \<stileturn> (\<lambda>r. \<langle>r = drop_bit (unat y) x\<rangle> \<star> \<psi> (drop_bit (unat y) x)) \<bowtie> \<rho> \<bowtie> \<theta>\<close>
    by (intro sstriple_frame_rule_no_return) (auto simp add: sstriple_word_shift_rightI)
  moreover from assms have \<open>\<Gamma> ; ?asm \<turnstile> wp_word_shift_right_expr1 x y \<stileturn> \<psi> \<bowtie> \<rho> \<bowtie> \<theta>\<close>
    by (intro sstriple_consequence[OF I]) auto
  ultimately show \<open>?asm \<longlongrightarrow> ?GOAL\<close>
    by (intro wp_is_weakest_precondition)
qed

urust_expr [abbrev] wp_word_shift_rightI_expr1
  (x, y)
  \<open> x >> y \<close>
...
lemma wp_word_shift_rightI [micro_rust_wp_intros]:
  ...
  assumes \<open>ucincl (\<psi> (drop_bit (unat y) x))\<close>
    ...
    shows \<open>\<phi> \<longlongrightarrow> \<W>\<P> \<Gamma> (wp_word_shift_rightI_expr1 x y) \<psi> \<rho> \<theta>\<close>
