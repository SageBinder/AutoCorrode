...
lemma urust_eval_action_add_no_wrap [urust_eval_action_simps]:
  ...
  shows \<open>((\<Gamma>, urust_eval_action_add_no_wrap_expr1 x y) \<diamondop>\<^sub>a \<sigma>) =
    ...
    and \<open>((\<Gamma>, urust_eval_action_add_no_wrap_expr1 x y) \<diamondop>\<^sub>v \<sigma>) =
            (if (unat x + unat y < 2 ^ LENGTH('l)) then {(x + y, \<sigma>)} else {})\<close>
    and \<open>((\<Gamma>, urust_eval_action_add_no_wrap_expr1 x y) \<diamondop>\<^sub>r \<sigma>) = {}\<close>
...
corollary urust_eval_predicate_add_no_wrap [urust_eval_predicate_simps]:
  ...
  shows \<open>\<sigma> \<leadsto>\<^sub>v \<langle>\<Gamma>, urust_eval_predicate_add_no_wrap_expr1 x y\<rangle> (v, \<sigma>') \<longleftrightarrow> (unat x + unat y < 2^LENGTH('l)) \<and> \<sigma> = \<sigma>' \<and> v = x + y\<close>
    and \<open>\<sigma> \<leadsto>\<^sub>r \<langle>\<Gamma>, urust_eval_predicate_add_no_wrap_expr1 x y\<rangle> (r, \<sigma>') \<longleftrightarrow> False\<close>
...
lemma urust_eval_action_mul_no_wrap [urust_eval_action_simps]:
  ...
  shows \<open>((\<Gamma>, urust_eval_action_mul_no_wrap_expr1 x y) \<diamondop>\<^sub>a \<sigma>) =
            (if (unat x * unat y < 2 ^ LENGTH('l)) then {} else { (Panic overflow_err_msg, \<sigma>) })\<close>
    and \<open>((\<Gamma>, urust_eval_action_mul_no_wrap_expr1 x y) \<diamondop>\<^sub>v \<sigma>) =
            (if (unat x * unat y < 2 ^ LENGTH('l)) then {(x * y, \<sigma>)} else {})\<close>
    and \<open>((\<Gamma>, urust_eval_action_mul_no_wrap_expr1 x y) \<diamondop>\<^sub>r \<sigma>) = {}\<close>
...
corollary urust_eval_predicate_mul_no_wrap [urust_eval_predicate_simps]:
  ...
  shows \<open>\<sigma> \<leadsto>\<^sub>v \<langle>\<Gamma>, urust_eval_predicate_mul_no_wrap_expr1 x y\<rangle> (v, \<sigma>') \<longleftrightarrow> (unat x * unat y < 2^LENGTH('l)) \<and> \<sigma> = \<sigma>' \<and> v = x * y\<close>
    and \<open>\<sigma> \<leadsto>\<^sub>r \<langle>\<Gamma>, urust_eval_predicate_mul_no_wrap_expr1 x y\<rangle> (r, \<sigma>') \<longleftrightarrow> False\<close>
...
lemma urust_eval_action_sub_no_wrap [urust_eval_action_simps]:
  ...
  shows \<open>((\<Gamma>, urust_eval_action_sub_no_wrap_expr1 x y) \<diamondop>\<^sub>a \<sigma>) =
    ...
    and \<open>((\<Gamma>, urust_eval_action_sub_no_wrap_expr1 x y) \<diamondop>\<^sub>v \<sigma>) =
            (if ( unat y \<le> unat x) then {(x - y, \<sigma>)} else {})\<close>
    and \<open>((\<Gamma>, urust_eval_action_sub_no_wrap_expr1 x y) \<diamondop>\<^sub>r \<sigma>) = {}\<close>
...
corollary urust_eval_predicate_sub_no_wrap [urust_eval_predicate_simps]:
  ...
  shows \<open>\<sigma> \<leadsto>\<^sub>v \<langle>\<Gamma>, urust_eval_predicate_sub_no_wrap_expr1 x y\<rangle> (v, \<sigma>') \<longleftrightarrow> (unat y \<le> unat x) \<and> \<sigma> = \<sigma>' \<and> v = x - y\<close>
    and \<open>\<sigma> \<leadsto>\<^sub>r \<langle>\<Gamma>, urust_eval_predicate_sub_no_wrap_expr1 x y\<rangle> (r, \<sigma>') \<longleftrightarrow> False\<close>
...
lemma urust_eval_action_div [urust_eval_action_simps]:
  ...
  shows \<open>((\<Gamma>, urust_eval_action_div_expr1 x y) \<diamondop>\<^sub>a \<sigma>) =
    ...
    and \<open>((\<Gamma>, urust_eval_action_div_expr1 x y) \<diamondop>\<^sub>v \<sigma>) =
            (if ( unat y \<noteq> 0) then {(x div y, \<sigma>)} else {})\<close>
    and \<open>((\<Gamma>, urust_eval_action_div_expr1 x y) \<diamondop>\<^sub>r \<sigma>) = {}\<close>
...
corollary urust_eval_predicate_div [urust_eval_predicate_simps]:
  ...
  shows \<open>\<sigma> \<leadsto>\<^sub>v \<langle>\<Gamma>, urust_eval_predicate_div_expr1 x y\<rangle> (v, \<sigma>') \<longleftrightarrow> (unat y \<noteq> 0) \<and> \<sigma> = \<sigma>' \<and> v = x div y\<close>
    and \<open>\<sigma> \<leadsto>\<^sub>r \<langle>\<Gamma>, urust_eval_predicate_div_expr1 x y\<rangle> (r, \<sigma>') \<longleftrightarrow> False\<close>
...
lemma urust_eval_action_mod [urust_eval_action_simps]:
  ...
  shows \<open>((\<Gamma>, urust_eval_action_mod_expr1 x y) \<diamondop>\<^sub>a \<sigma>) =
            (if ( unat y \<noteq> 0) then {} else { (Panic (String.implode ''division by zero''), \<sigma>) })\<close>
    and \<open>((\<Gamma>, urust_eval_action_mod_expr1 x y) \<diamondop>\<^sub>v \<sigma>) =
            (if ( unat y \<noteq> 0) then {(x mod y, \<sigma>)} else {})\<close>
    and \<open>((\<Gamma>, urust_eval_action_mod_expr1 x y) \<diamondop>\<^sub>r \<sigma>) = {}\<close>
...
corollary urust_eval_predicate_mod [urust_eval_predicate_simps]:
  ...
  shows \<open>\<sigma> \<leadsto>\<^sub>v \<langle>\<Gamma>, urust_eval_predicate_mod_expr1 x y\<rangle> (v, \<sigma>') \<longleftrightarrow> (unat y \<noteq> 0) \<and> \<sigma> = \<sigma>' \<and> v = x mod y\<close>
    and \<open>\<sigma> \<leadsto>\<^sub>r \<langle>\<Gamma>, urust_eval_predicate_mod_expr1 x y\<rangle> (r, \<sigma>') \<longleftrightarrow> False\<close>
...
lemma urust_eval_action_shift_left [urust_eval_action_simps]:
  ...
  shows \<open>((\<Gamma>, urust_eval_action_shift_left_expr1 x y) \<diamondop>\<^sub>a \<sigma>) =
            (if unat y < LENGTH('l0) then {} else {(Panic overflow_err_msg, \<sigma>)})\<close>
    and \<open>((\<Gamma>, urust_eval_action_shift_left_expr1 x y) \<diamondop>\<^sub>v \<sigma>) =
            (if unat y < LENGTH('l0) then {(push_bit (unat y) x, \<sigma>)} else {})\<close>
    and \<open>((\<Gamma>, urust_eval_action_shift_left_expr1 x y) \<diamondop>\<^sub>r \<sigma>) = {}\<close>
...
corollary urust_eval_predicate_shift_left [urust_eval_predicate_simps]:
  fixes x :: \<open>'l0::{len} word\<close>
    and y :: \<open>64 word\<close>
  shows \<open>\<sigma> \<leadsto>\<^sub>v \<langle>\<Gamma>, urust_eval_predicate_shift_left_expr1 x y\<rangle> (v, \<sigma>') \<longleftrightarrow>
            unat y < LENGTH('l0) \<and> \<sigma> = \<sigma>' \<and> v = push_bit (unat y) x\<close>
    and \<open>\<sigma> \<leadsto>\<^sub>r \<langle>\<Gamma>, urust_eval_predicate_shift_left_expr1 x y\<rangle> (r, \<sigma>') \<longleftrightarrow> False\<close>
...
lemma urust_eval_action_shift_right [urust_eval_action_simps]:
  fixes x :: \<open>'l0::{len} word\<close>
    and y :: \<open>64 word\<close>
  shows \<open>((\<Gamma>, urust_eval_action_shift_right_expr1 x y) \<diamondop>\<^sub>a \<sigma>) =
            (if unat y < LENGTH('l0) then {} else {(Panic overflow_err_msg, \<sigma>)})\<close>
    and \<open>((\<Gamma>, urust_eval_action_shift_right_expr1 x y) \<diamondop>\<^sub>v \<sigma>) =
            (if unat y < LENGTH('l0) then {(drop_bit (unat y) x, \<sigma>)} else {})\<close>
    and \<open>((\<Gamma>, urust_eval_action_shift_right_expr1 x y) \<diamondop>\<^sub>r \<sigma>) = {}\<close>
...
corollary urust_eval_predicate_shift_right [urust_eval_predicate_simps]:
  fixes x :: \<open>'l0::{len} word\<close>
    and y :: \<open>64 word\<close>
  shows \<open>\<sigma> \<leadsto>\<^sub>v \<langle>\<Gamma>, urust_eval_predicate_shift_right_expr1 x y\<rangle> (v, \<sigma>') \<longleftrightarrow>
            unat y < LENGTH('l0) \<and> \<sigma> = \<sigma>' \<and> v = drop_bit (unat y) x\<close>
    and \<open>\<sigma> \<leadsto>\<^sub>r \<langle>\<Gamma>, urust_eval_predicate_shift_right_expr1 x y\<rangle> (r, \<sigma>') \<longleftrightarrow> False\<close>
...
lemma urust_eval_action_xor [urust_eval_action_simps]:
  ...
  shows \<open>((\<Gamma>, urust_eval_action_xor_expr1 x y) \<diamondop>\<^sub>a \<sigma>) = {}\<close>
    and \<open>((\<Gamma>, urust_eval_action_xor_expr1 x y) \<diamondop>\<^sub>v \<sigma>) = {(x XOR y, \<sigma>)}\<close>
    and \<open>((\<Gamma>, urust_eval_action_xor_expr1 x y) \<diamondop>\<^sub>r \<sigma>) = {}\<close>
...
corollary urust_eval_predicate_xor [urust_eval_predicate_simps]:
  ...
  shows \<open>\<sigma> \<leadsto>\<^sub>v \<langle>\<Gamma>, urust_eval_predicate_xor_expr1 x y\<rangle> (v, \<sigma>') \<longleftrightarrow> \<sigma> = \<sigma>' \<and> v = x XOR y\<close>
    and \<open>\<sigma> \<leadsto>\<^sub>r \<langle>\<Gamma>, urust_eval_predicate_xor_expr1 x y\<rangle> (r, \<sigma>') \<longleftrightarrow> False\<close>
...
lemma urust_eval_action_or [urust_eval_action_simps]:
  ...
  shows \<open>((\<Gamma>, urust_eval_action_or_expr1 x y) \<diamondop>\<^sub>a \<sigma>) = {}\<close>
    and \<open>((\<Gamma>, urust_eval_action_or_expr1 x y) \<diamondop>\<^sub>v \<sigma>) = {(x OR y, \<sigma>)}\<close>
    and \<open>((\<Gamma>, urust_eval_action_or_expr1 x y) \<diamondop>\<^sub>r \<sigma>) = {}\<close>
by (simp add: urust_eval_action_bind micro_rust_simps urust_eval_action_literal
  urust_eval_action_abort urust_eval_action_call)+
...
corollary urust_eval_predicate_or [urust_eval_predicate_simps]:
  ...
  shows \<open>\<sigma> \<leadsto>\<^sub>v \<langle>\<Gamma>, urust_eval_predicate_or_expr1 x y\<rangle> (v, \<sigma>') \<longleftrightarrow> \<sigma> = \<sigma>' \<and> v = x OR y\<close>
    and \<open>\<sigma> \<leadsto>\<^sub>r \<langle>\<Gamma>, urust_eval_predicate_or_expr1 x y\<rangle> (r, \<sigma>') \<longleftrightarrow> False\<close>
...
lemma urust_eval_action_and [urust_eval_action_simps]:
  ...
  shows \<open>((\<Gamma>, urust_eval_action_and_expr1 x y) \<diamondop>\<^sub>a \<sigma>) = {}\<close>
    and \<open>((\<Gamma>, urust_eval_action_and_expr1 x y) \<diamondop>\<^sub>v \<sigma>) = {(x AND y, \<sigma>)}\<close>
    and \<open>((\<Gamma>, urust_eval_action_and_expr1 x y) \<diamondop>\<^sub>r \<sigma>) = {}\<close>
by (simp add: urust_eval_action_bind micro_rust_simps urust_eval_action_literal
  urust_eval_action_abort urust_eval_action_call)+
...
corollary urust_eval_predicate_and [urust_eval_predicate_simps]:
  ...
  shows \<open>\<sigma> \<leadsto>\<^sub>v \<langle>\<Gamma>, urust_eval_predicate_and_expr1 x y\<rangle> (v, \<sigma>') \<longleftrightarrow> \<sigma> = \<sigma>' \<and> v = x AND y\<close>
    and \<open>\<sigma> \<leadsto>\<^sub>r \<langle>\<Gamma>, urust_eval_predicate_and_expr1 x y\<rangle> (r, \<sigma>') \<longleftrightarrow> False\<close>
...
lemma urust_eval_action_not [urust_eval_action_simps]:
  fixes x y :: \<open>'l::{len} word\<close>
  shows \<open>((\<Gamma>, urust_eval_action_not_expr1 x) \<diamondop>\<^sub>a \<sigma>) = {}\<close>
    and \<open>((\<Gamma>, urust_eval_action_not_expr1 x) \<diamondop>\<^sub>v \<sigma>) = {(NOT x, \<sigma>)}\<close>
    and \<open>((\<Gamma>, urust_eval_action_not_expr1 x) \<diamondop>\<^sub>r \<sigma>) = {}\<close>
by (simp add: urust_eval_action_bind micro_rust_simps urust_eval_action_literal
  urust_eval_action_abort urust_eval_action_call)+
...
corollary urust_eval_predicate_not [urust_eval_predicate_simps]:
  fixes x y :: \<open>'l::{len} word\<close>
  shows \<open>\<sigma> \<leadsto>\<^sub>v \<langle>\<Gamma>, urust_eval_predicate_not_expr1 x\<rangle> (v, \<sigma>') \<longleftrightarrow> \<sigma> = \<sigma>' \<and> v = NOT x\<close>
    and \<open>\<sigma> \<leadsto>\<^sub>r \<langle>\<Gamma>, urust_eval_predicate_not_expr1 x\<rangle> (r, \<sigma>') \<longleftrightarrow> False\<close>
    and \<open>\<sigma> \<leadsto>\<^sub>a \<langle>\<Gamma>, urust_eval_predicate_not_expr1 x\<rangle> (a, \<sigma>') \<longleftrightarrow> False\<close>
