...
| D-21 | Struct-expression positive coverage is in `Parser_Expr_Conformance_Tests.thy`; negative coverage is in `Parser_Rejection_Tests.thy`. |
...
| T-39 | The struct-expression matrices in `Parser_Expr_Conformance_Tests.thy` and `Parser_Rejection_Tests.thy` preserve frontend-compatible syntax-only labels while metadata-correct semantics remain future work. |
