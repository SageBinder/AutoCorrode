...
struct
  ...
  fun lower_expression ctxt environment expression =
    (case expression of
     ...
     | UE_Let binding =>
         lower_let ctxt environment binding
     | UE_LetMut binding =>
         lower_mutable_let ctxt environment binding
     | UE_Const binding =>
         lower_let ctxt environment binding
  ...
  and lower_place ctxt environment place =
    (case place of
       UP_Ident identifier =>
         R.literal_identifier ctxt environment identifier
     ...
     | UP_Field (base, name, pos) =>
         R.field_expression ctxt
           (lower_place ctxt environment base) name pos
     | UP_Antiq source =>
         R.parse_antiquotation ctxt environment source)

  and lower_let ctxt environment (pattern, rhs, body) =
    let
      val lowered_rhs = lower_expression ctxt environment rhs
      val (abstraction, body_environment) =
        P.bind_irrefutable ctxt environment pattern
      val lowered_body =
        lower_expression ctxt body_environment body
  ...
  and lower_mutable_let ctxt environment (pattern, rhs, body, mutable_pos) =
    let
      fun allocate () =
        let
          val lowered_rhs =
            T.allocate_reference mutable_pos
              (lower_expression ctxt environment rhs)
          val (abstraction, body_environment) =
            P.bind_irrefutable ctxt environment pattern
          val lowered_body =
            lower_expression ctxt body_environment body
    ...
    in
      (case pattern of
         P_Ident _ => allocate ()
       | P_Wild _ => allocate ()
       | P_Tuple _ => lower_let ctxt environment (pattern, rhs, body)
       | _ =>
  ...
  and lower_match ctxt environment (flavour, scrutinee, arms, pos) =
    let
      val selected = P.select_match_flavour flavour arms pos
      val lowered_scrutinee =
        lower_expression ctxt environment scrutinee
    in
      (case selected of
         MF_Switch =>
           let
             ...
             fun lower_arm alternative =
               let
                 ...
                 val lowered_guard =
                   Option.map
                     (fn (guard, _) =>
                       lower_expression ctxt arm_environment guard)
                     (P.prepared_guard prepared)
                 val lowered_body =
                   lower_expression ctxt arm_environment
                     (P.prepared_body prepared)
