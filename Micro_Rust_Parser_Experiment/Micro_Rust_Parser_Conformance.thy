(* Conformance of the custom µRust parser against the inner-syntax frontend.
   ...
   For every implemented construct we `micro_rust_expr NAME <src>` (defining a constant via the custom
   ...
   Definability note (R1): `micro_rust_expr` DEFINES a constant, so a `src` leaving a free type variable
...
micro_rust_expr and_ff \<open> False && False \<close>
...
micro_rust_expr or_ft \<open> False || True \<close>
...
micro_rust_expr lop_div \<open> let a = \<llangle>12 :: 32 word\<rrangle>; let b = \<llangle>4 :: 32 word\<rrangle>; a / b \<close>
...
micro_rust_expr lop_mod \<open> let a = \<llangle>17 :: 32 word\<rrangle>; let b = \<llangle>5 :: 32 word\<rrangle>; a % b \<close>
...
micro_rust_expr lop_bor \<open> let a = \<llangle>0xF0 :: 32 word\<rrangle>; let b = \<llangle>0x0F :: 32 word\<rrangle>; a | b \<close>
...
micro_rust_expr lop_bxor \<open> let a = \<llangle>0xFF :: 32 word\<rrangle>; let b = \<llangle>0x0F :: 32 word\<rrangle>; a ^ b \<close>
...
micro_rust_expr lop_shr \<open> let a = \<llangle>16 :: 32 word\<rrangle>; a >> \<llangle>2 :: 64 word\<rrangle> \<close>
...
micro_rust_expr ext_if_cond \<open> if if \<llangle>True\<rrangle> { \<llangle>True\<rrangle> } else { \<llangle>False\<rrangle> } { () } else { () } \<close>  \<comment>\<open> if-expr as condition \<close>
...
micro_rust_expr call3 \<open> cf3(\<llangle>1 :: 64 word\<rrangle>, \<llangle>2 :: 64 word\<rrangle>, \<llangle>3 :: 64 word\<rrangle>) \<close>
...
micro_rust_expr call4 \<open> cf4(\<llangle>1 :: 64 word\<rrangle>, \<llangle>2 :: 64 word\<rrangle>, \<llangle>3 :: 64 word\<rrangle>, \<llangle>4 :: 64 word\<rrangle>) \<close>
...
micro_rust_expr call5 \<open> cf5(\<llangle>1 :: 64 word\<rrangle>, \<llangle>2 :: 64 word\<rrangle>, \<llangle>3 :: 64 word\<rrangle>, \<llangle>4 :: 64 word\<rrangle>, \<llangle>5 :: 64 word\<rrangle>) \<close>
...
begin
micro_rust_expr call_funit \<open> uf(()) \<close>
lemma \<open> call_funit = \<lbrakk> uf(()) \<rbrakk> \<close> unfolding call_funit_def by (rule refl)

micro_rust_expr call_gunitbool \<open> ug((), True) \<close>
...
micro_rust_expr mcall0 \<open> \<llangle>5 :: 64 word\<rrangle>.cf1() \<close>
lemma \<open> mcall0 = \<lbrakk> \<llangle>5 :: 64 word\<rrangle>.cf1() \<rbrakk> \<close> unfolding mcall0_def by (rule refl)

micro_rust_expr mcall1 \<open> \<llangle>1 :: 64 word\<rrangle>.cf2(\<llangle>2 :: 64 word\<rrangle>) \<close>
...
micro_rust_expr mcall_on_call \<open> cf1(\<llangle>1 :: 64 word\<rrangle>).cf1() \<close>
lemma \<open> mcall_on_call = \<lbrakk> cf1(\<llangle>1 :: 64 word\<rrangle>).cf1() \<rbrakk> \<close> unfolding mcall_on_call_def by (rule refl)

micro_rust_expr mcall_chain \<open> \<llangle>5 :: 64 word\<rrangle>.cf1().cf1() \<close>
lemma \<open> mcall_chain = \<lbrakk> \<llangle>5 :: 64 word\<rrangle>.cf1().cf1() \<rbrakk> \<close> unfolding mcall_chain_def by (rule refl)

micro_rust_expr mcall_operand \<open> \<llangle>1 :: 64 word\<rrangle>.cf1() + \<llangle>2 :: 64 word\<rrangle> \<close>
...
begin
micro_rust_expr mcall_ctx \<open> m.h() \<close>
lemma \<open> mcall_ctx = \<lbrakk> m.h() \<rbrakk> \<close> unfolding mcall_ctx_def by (rule refl)

micro_rust_expr mcall_if \<open> if m.h() == n { m } else { n } \<close>
...
micro_rust_expr rc_arg_op \<open> cf1(\<llangle>1 :: 64 word\<rrangle> + \<llangle>2 :: 64 word\<rrangle>) \<close>
...
micro_rust_expr rc_call_plus_call \<open> cf1(\<llangle>10 :: 64 word\<rrangle>) + cf1(\<llangle>20 :: 64 word\<rrangle>) \<close>
...
micro_rust_expr rc_call_times \<open> cf2(\<llangle>1 :: 64 word\<rrangle>, \<llangle>2 :: 64 word\<rrangle>) * \<llangle>3 :: 64 word\<rrangle> \<close>
...
micro_rust_expr rc_bang_cmp \<open> !(cf1(\<llangle>1 :: 64 word\<rrangle>) == cf1(\<llangle>2 :: 64 word\<rrangle>)) \<close>
...
micro_rust_expr rc_call_let \<open> let r = cf1(\<llangle>7 :: 64 word\<rrangle>); r \<close>
lemma \<open> rc_call_let = \<lbrakk> let r = cf1(\<llangle>7 :: 64 word\<rrangle>); r \<rbrakk> \<close> unfolding rc_call_let_def by (rule refl)

micro_rust_expr rc_call_if \<open> if \<llangle>True\<rrangle> { cf0() } else { cf1(\<llangle>1 :: 64 word\<rrangle>) } \<close>
...
micro_rust_expr rc_call_block \<open> { cf0(); cf1(\<llangle>1 :: 64 word\<rrangle>) } \<close>
lemma \<open> rc_call_block = \<lbrakk> { cf0(); cf1(\<llangle>1 :: 64 word\<rrangle>) } \<rbrakk> \<close> unfolding rc_call_block_def by (rule refl)

micro_rust_expr rc_if_arg \<open> cf1((if \<llangle>True\<rrangle> { \<llangle>1 :: 64 word\<rrangle> } else { \<llangle>2 :: 64 word\<rrangle> })) \<close>
...
micro_rust_expr rc_deep3 \<open> cf1(cf1(cf1(\<llangle>1 :: 64 word\<rrangle>))) \<close>
lemma \<open> rc_deep3 = \<lbrakk> cf1(cf1(cf1(\<llangle>1 :: 64 word\<rrangle>))) \<rbrakk> \<close> unfolding rc_deep3_def by (rule refl)

micro_rust_expr rc_two_call_args \<open> cf2(cf2(\<llangle>1 :: 64 word\<rrangle>, \<llangle>2 :: 64 word\<rrangle>), cf1(\<llangle>3 :: 64 word\<rrangle>)) \<close>
...
micro_rust_expr rc_cap_args \<open> let a = \<llangle>10 :: 64 word\<rrangle>; let b = \<llangle>20 :: 64 word\<rrangle>; cf2(\<llangle>a\<rrangle>, \<llangle>b\<rrangle>) \<close>
...
micro_rust_expr rc_mcall_let_recv \<open> let x = \<llangle>5 :: 64 word\<rrangle>; x.cf1() \<close>
...
micro_rust_expr rc_mchain2 \<open> \<llangle>1 :: 64 word\<rrangle>.cf2(\<llangle>2 :: 64 word\<rrangle>).cf1() \<close>
...
micro_rust_expr rc_m_on_call \<open> cf1(\<llangle>1 :: 64 word\<rrangle>).cf2(\<llangle>2 :: 64 word\<rrangle>) \<close>
...
micro_rust_expr rc_m_call_arg \<open> \<llangle>1 :: 64 word\<rrangle>.cf2(cf1(\<llangle>2 :: 64 word\<rrangle>)) \<close>
...
micro_rust_expr rc_m_prec \<open> \<llangle>1 :: 64 word\<rrangle>.cf1() + \<llangle>2 :: 64 word\<rrangle> * \<llangle>3 :: 64 word\<rrangle> \<close>
...
incompatible types) the case is recorded via whichever of \<open>micro_rust_expr\<close> (parser accepts) or a golden
...
micro_rust_expr cap_const_fst \<open> let fst = \<llangle>5 :: nat\<rrangle>; \<llangle>fst\<rrangle> \<close>  \<comment>\<open> binder name = HOL \<open>Product_Type.fst\<close> \<close>
...
micro_rust_expr cap_const_deep \<open> let id = \<llangle>5 :: nat\<rrangle>; \<llangle>id + 1\<rrangle> \<close>  \<comment>\<open> buried capture of a const-named binder \<close>
...
micro_rust_expr cap_notation \<open> let myReg = \<llangle>5 :: nat\<rrangle>; \<llangle>myReg\<rrangle> \<close>  \<comment>\<open> binder name = a registered notation surface name (guard) \<close>
